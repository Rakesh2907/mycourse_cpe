            ...
            $('.del').click(function(){ 
			  var qid=$(this).attr('id');
                ...
                bootbox.confirm("Are you sure?", function(result) {
					...
					if(result == true)
					{
						$('#course_completed').modal();
						$('#course_loader').show();
				   	$.ajax({
						url: SITE_URL+"customer_con/make_course_complete/"+qid, 
						success: function(result){
						if(result == 1){
							$.ajax({
								url: SITE_URL+"customer_con/amazon_upload_certificate/"+qid,
								success: function(result){
